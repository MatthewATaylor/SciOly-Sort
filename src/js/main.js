...
//----EVENTS----
class SOEvent {
    constructor(name, block, maxStudents, studentNames) {
        this.name = name;
        this.block = block;
...
//----STUDENTS----
class Student {
    constructor(email, firstName, lastName, grade, parentMentor, topEvents, testScore, sortScore) {
        this.email = email;
...
function parseEvents() {
    ...
    for (let i = 1; i < eventLines.length; ++i) {
        ...
        let eventStudentNames = [];
        for (let j = 3; j < eventTokens.length; ++j) {
            if (eventTokens[j].trim().length > 0) {
                eventStudentNames.push(eventTokens[j]);
            }
        }
        let event = new SOEvent(eventTokens[0], eventTokens[1], parseInt(eventTokens[2]), eventStudentNames);
        events.push(event);
    }
    return true;
}
...
function parseStudents() {
    ...
    for (let i = 1; i < registrationLines.length; ++i) {
        ...
        if (isNaN(grade) || (grade !== 6 && grade !== 7 && grade !== 8)) {
            displayMessage("Invalid grade \"" + gradeToken + "\" for student \"" +
        ...
        if (parentMentorToken.toLowerCase() === "yes") {
            ...
            displayMessage("Invalid parent mentor response \"" + parentMentorToken +
        ...
        let topEvents = new Array(numEvents);
        ...
        for (let i = 6; i < registrationTokens.length; ++i) {
            ...
            if (isNaN(eventRank) || eventRank <= 0 || eventRank > numEvents) {
                displayMessage("Invalid event rank \"" + eventRankToken + "\" for student \"" +
                    firstNameToken + " " + lastNameToken + "\"");
                return false;
            }
        ...
        let student = new Student(emailToken, firstNameToken, lastNameToken, grade, parentMentor, topEvents, testScore, 0);
...
function setSortScores() {
    ...
    let parentMentorSlider = document.getElementById("mentor-slider");
...
function assignEvents() {
    let grade6Slider = document.getElementById("grade6-slider");
    let grade7Slider = document.getElementById("grade7-slider");
    let grade8Slider = document.getElementById("grade8-slider");
    ...
    let maxEventsSlider = document.getElementById("event-slider");
    ...
    for (let i = 0; i < students.length; ++i) {
        for (let j = 0; j < students[i].topEvents.length; ++j) {
            ...
            let numEventStudents = currentEvent.studentNames.length +
